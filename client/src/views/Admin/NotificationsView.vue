...
function getNotificationText(n: NotificationData): string {
  if (n.type === "JobStatusChanged") {
    ...
    return `Job listing "${n.data.job_title}" status updated to ${status}.` + (n.data.reason ? ` Reason: ${n.data.reason}` : "");
  }
  if (n.type === "UserStatusChanged") {
    return `User account #${n.data.user_id} status changed to ${n.data.status}.`;
  }
