...
function getErrorMessage(error: unknown) {
  ...
  return firstValidationError || response?.message || 'Failed to update your account.';
}
        ...
        <aside class="rounded-2xl border border-outline-variant/50 bg-surface-container p-6 shadow-sm">
          <div class="flex flex-col items-center text-center">
            <div class="mb-4 flex h-28 w-28 items-center justify-center overflow-hidden rounded-full border-4 border-white bg-surface-container-lowest shadow-sm">
              ...
              <span v-else class="font-headline-xl text-headline-xl text-primary">{{ user.name?.charAt(0).toUpperCase() || 'E' }}</span>
            </div>

            <h2 class="font-headline-md text-headline-md text-on-surface">{{ user.name || 'Employer' }}</h2>
          ...
          <div class="rounded-2xl border border-outline-variant/50 bg-surface-container p-6 shadow-sm">
            ...
            <div class="grid gap-4 md:grid-cols-2">
              ...
              <label class="grid gap-2">
                ...
                <input v-model="form.passwordConfirmation" class="rounded-lg border border-outline-variant bg-surface px-4 py-3 text-on-surface outline-none focus:border-primary" type="password" />
              </label>
            </div>
          </div>
